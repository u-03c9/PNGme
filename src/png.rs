...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        let header: [u8; 8] = buffer;
        if header != Self::STANDARD_HEADER {
            return Err(Error::from("Invalid file header"));
        }

        let mut data_length_buffer: [u8; 4] = Default::default();
        let mut chunk_type_buffer: [u8; 4] = Default::default();
        let mut data_buffer: [u8; 1] = Default::default();
        ...
        loop {
            ...
            if reader.read_exact(&mut data_length_buffer).is_err() {
                break;
            };

            let data_length = u32::from_be_bytes(data_length_buffer);
            chunk.extend(data_length_buffer.to_vec());

            reader.read_exact(&mut chunk_type_buffer)?;
            chunk.extend(chunk_type_buffer.to_vec());

            for _ in 0..data_length {
                reader.read_exact(&mut data_buffer)?;
                chunk.extend(data_buffer.to_vec());
            }

            reader.read_exact(&mut crc_buffer)?;
            chunk.extend(crc_buffer.to_vec());

            let chunk = Chunk::try_from(chunk.as_ref()).expect("Invalid chunk");
            chunks.push(chunk);
        }
